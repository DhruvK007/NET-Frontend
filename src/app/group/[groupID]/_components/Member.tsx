...
export const Member = ({
  ...
  return (
    <CardContent
      className="relative flex w-full flex-col md:flex-row gap-3 rounded-xl border p-4 shadow dark:bg-Neutral-100 mb-2 md:mb-4 dark:border-neutral-700"
      onMouseEnter={() => setShowTooltip(true)}
      onMouseLeave={() => setShowTooltip(false)}
    >
      <section className="flex flex-row items-center justify-between w-full gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span
              className={`text-xs md:text-sm ${
                status === "settled up" ? "text-gray-500" : amountColor
              }`}
            >
              {status}
            </span>
        ...
        <div
          className={`flex items-center text-base md:text-lg font-semibold mt-2 md:mt-0 ${
            status === "settled up" ? "text-gray-500" : amountColor
          }`}
        >
          {status !== "settled up" && "₹"}
